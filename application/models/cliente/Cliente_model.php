...
//**************inicio de calculo %grasa,masa magra,ccd y tmb *****************************
		...
		/*$datos["id_cliente"] = $this->session->userdata('id');
		...
		$datos["cintura"] = $this->input->post('Cintura');

		$datos["cuello"] = $this->input->post('Cuello');

		$datos["cadera"] = $this->input->post('cadera');

		$datos["sexo"] = $this->input->post('sexo');

		$datos["nivel_actividad"] = $this->input->post('factor');

		$datos["fecha_registro"] = "now()";

		$datos["imc"] = $imc;

		$datos["porcentaje_grasa"] = $porcentajeGrasa2;

		$datos["masa_magra"] = $Masacorporalmagra2;

		$datos["tmb"] = $tmb;

		$datos["ccd"] = $ccd;

        return $this->db->insert('estado_fisico',$datos);	*/
		...
		$ccd.")";
		 ...
		 $mama=$this->db->query($queryInsert);
	...
	function rescatar_total_calorias(){
		...
		$query = 	$query  = $this->db->query("select insert_cont_detalle(".$cliente.",".$id_alimento.",".$id_comida.",".$cantidad.",".$total_calorias.") as valida  from dual");
